...
class UserProfile(models.Model):
    ...
    memo = models.TextField('备注', blank=True, null=True, default=True)
...
class Customer(models.Model):
    ...
    class_type = models.CharField('班级类型', max_length=64, choices=class_type_choices, default='uflltime')
    ...
    date = models.DateTimeField('资讯日期', auto_now_add=True)
    ...
    def __str__(self):
        return '{} QQ:{}'.format(self.name, self.qq)
...
class ClassList(models.Model):
    ...
    #  obj.get_FIELD_display()  拿到choices选项的具体内容
    def __str__(self):
        return '{}{}({})'.format(self.get_course_display(), self.semester, self.campuses)

    # 通过班级反向查询管理对象拿到老师列表,循环列表,获得每一个老师对象, 通过调用str方法获得名称
...
class PaymentRecord(models.Model):
    ...
    course = models.CharField('课程名称', choices=course_choices, max_length=64, blank=True, null=True)
    ...
    confirm_user = models.ForeignKey(verbose_name='确认人', to='UserProfile', related_name='confirms',
